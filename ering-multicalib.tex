...
A Gaussian Process (GP) metamodel is then used to approximate the full-complexity model and is leveraged to perform stochastic analysis through Bayesian calibration of ten sensitive roughness zones, five of which correspond to instream morphological units (MUs).The method assesses riverbed roughness variability as an essential driver of enhanced model calibration and uncertainty reduction. The calibration technique also takes into account the critical Shields parameter as a highly sensitive parameter for bedload sediment transport and thereby a trigger for changes in hydrodynamic patterns.To make stochastic calibration feasible and incorporate all available data, this study employs a multi-output Gaussian Process approach that simultaneously models two correlated output variables ( water depth and scalar velocity) which both serve as calibration targets. The key idea is that the multitask surrogate leverages the physical relationship between water depth and scalar velocity, as governed by the momentum and energy equations. Furthermore, the metamodel uses a Bayesian Active Learning (BAL) approach proposed by \citeA{oladyshkin2020bayesian3} to add additional training points based on relative entropy to improve the accuracy of the metamodel after a desired number of iterations. 

Through this approach, the study aims to answer two research questions: \textit{First, how does a morphological unit–varying roughness of the riverbed, aided by a trained multi-output Gaussian Process metamodel, increase the effectiveness of model calibration and uncertainty quantification of model parameters?} \textit{Second, to what extent does a multitask Gaussian Process metamodel improve the learning process and accurate reproduction of two correlated variables compared to a single-output Gaussian Process?}


We hypothesize that sensitive calibration model parameters such as a morphological unit-based roughness coefficients and critical Shields parameter, aided by data-driven and machine lerning techniques, imroves the uncertainty quantification and narrows the posterior distribution functions for highly uncertain parameters. Additionally, we hypothesize that the multitask Gaussian Process (MTGP) trained with two correlated variables outperforms the single-output Gaussian Process (GP) by providing higher model accuracy and narrower confidence intervals, especially in scenarios where the target variables exhibit a natural correlation.
  

To this end, the sections  \ref{subsec:Sec2.1} through \ref{sec:Sec2.3} introduces the area of study and the numerical model configuration along with the available data used for the present project. \ref{sec:Sec2.4} to \ref{sec:sec2.5} show the selected calibration parameters describing the methodology for delineation of morphological units. \ref{sec:sec2.6} provides the basic concepts of surrogate-assisted Bayesian Calibration using Gaussian Process metamodels. Finally, \ref{Results} shows the obtained results according to the assessment methodology presentd in sections \ref{sec:Sec2.7} and \ref{sec:sec2.8}, concluding with the final remarks and suggestions for further improvements.
...
A metamodel using Gaussian Process Regression is employed to approximate a complex hydro-morphodynamic model and perform Bayesian model calibration along with uncertanty quantification of the calibration parameters. The approach centers primarily in the estimation of two correlated variables that share underlying dependencies such as velocity and water depth per measurement location. Typically, one can predict each output of interest separately by training single output Gaussian Process (SOGP) surrogates denoted as:
...
\citeA{oladyshkin2020bayesian3} suggest that a stopping criterion for BAL iterations is reached when the surrogate model response $\mathcal{M}$ converges toward the full-complexity model response $M_c$ meaning Relative Entropy (RE) and Bayesian Model Evidence (BME) also converge and stabilize over a plateau when plotted against the number of BAL iterations.
...
\subsection{Stochastic calibration assessment and uncertainty quantification}
{\label{sec:Sec2.7}} 

The calibration is performed using velocities and water depths collected at 37 measurement points. Each run of the hydro-morphodynamic model takes approximately 20 minutes on a 16-core PC. Based on the two calibration targets retrieved from an initial model run for its pre- and post-flush scenarios, and following the methodology explained in Section\ref{sec:Sec2.5.1} we delineated morphological units to subdivide the riverbed into spatially varying roughness zones, as shown in Fig \ref{fig:MU}.

The surrogate-assisted calibration starts with the selection of 10 sensitive model parameters: 9 roughness zones and 1 sensitive model parameter related to bedload sediment transport, as previously shown in Table \ref{tab:roughness_zones}. Among all the considered roughness zones, five correspond to morphological units such as pool, slackwater, glides, riffles, and runs covering the entire simulation domain. Additionally, backwater and wake regions surrounding the large wood logs, wood log regions (LWs), and tree stem roughness zones were taken as sensitive for model calibration parameters.

To account for bedload sediment transport effects and their relation to morphological evolution and hydrodynamic patters variability, the critical Shields parameter for the first sediment class in the riverbed configuration was considered a calibration parameter. The critical Shields parameters of the remaining sediment classes did not exhibit sensitive behavior during an initial sensitivity analysis and therefore not considered for model calibration. The calibration framework proceeds as follows:

\textbf{First}, we performed 25 training runs to initially train the metamodel. Three GP metamodels were trained (SOGP -scalar velocity, SOGP -water depth and MOGO -water depth \& scalar velocity ). The surrogates´ are trained at the 37 measurement points.

\textbf{Second}, we fixed 75 BAL iterations, adequate for RE and BME stabilization. The Bayesian Active Learning (BAL) stopping criterion was based on previous test cases in which a clear stabilization of the RMSE and BME metrics was not observed. Upon completion of the BAL iterations, we quantyfied model parameter uncertainty via Bayesian inference using the last-trained SOGP and MOGP surrogate metamodels to obtain posterior distributions of the model calibration parameters. This allows us to quantify i) the metamodel which gives a clear uncertainty quantification of the selected model parameters ii) the clear posterior distributions that provide high probability regions that potentially fit better the measured data. 

\textbf{Third}, after the final MOGP and the last BAL iteration were achieved, we assume the 'best performing' set of parameters as the maximum a posteriori (MAP) estimate from the posterior distributions after Bayesian inference using the final MOGP metamodel. To test the effectiveness of the proposed calibration methodology, the best-performing set of parameters is evaluated using both the complex model and the MOGP model. The results are then compared to those of a calibrated model that assumes a uniform Nikuradse roughness coefficient \(K_{s_{UN}}\) along the entire riverbed
...
\label{sec:sec2.8}
To compare both Single-Output (SO) and Multi-Output (MO) Gaussian Process metamodels in accurately approximating the full complex hydro-morphodynamic model, the study evaluates the evolution of three relevant metrics at different stages of training. In particular, the study computes the Mean Absolute Error (MAE), the Pearson correlation coefficient (r), and the mean of the confidence interval ranges across the measurement points for each set of training points. For initial training, the study assesses the metamodel’s accuracy to compute both response variables---velocity and water depth---depending solely on the number of initial training points, ranging from 5 to 25. Subsequently, the study tests the effectiveness of Bayesian Active Learning (BAL) from 25 to 100 training points to further assess the improvement in surrogate accuracy as the training set expands.  
...
\subsection{Effects of Increasing training points in Single and Multioutput Gaussian Processes}
