...
\subsection{Bayesian calibration and uncertainty quantification of model parameters}{\label{sec:Sec2.8}} 

The calibration is performed using  velocities and water depths collected at 37 measurement points. Each run of the hydro-morphodynamic model takes aproximately 20 minutes with 16 core PC. From a first model run and from pre and post flush scenarios at the base flow of , velocities and water depths are eThe surrgate assisted calibratins starts by 10 sensitiv model parameters, 9 roughness yones and 1 sentive model parameter for bedload sediment transport. Among them there are the five morophological units, backwater and wake regions in the soroundings of the wood logs, woogd logs regions and tree stems. Additionally, the ciritical Shields parameter considered a sensitive calibration parameter for bedload sediment transport is taken into account. At the very begining, we defined 25 training runs for first training the metamodel before starting with 35 BAL iterations. We train a multitask Gaussian Process whose response is the 2 calibration quantities scalar velocity and water depth at the 37 measurement points. To assess the calibration, we have selected the RMSE and MAE as the most revelant metrics for comparison between measured data and model output. Additionally, we quantify parameter model uncertainty via Bazeisian inference using the trained surrogate metamodel to obtain posterior distributions of the calibration parameters. At the end, we select the maximum a posteriori set of parameters which is the calibrated set of parameters at the 35th BAL iteration. . 
